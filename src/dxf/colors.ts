 ...
 *
 * DXF entities usually carry a palette index rather than an RGB value, so the
 * standard 256-entry table has to be reproduced here to draw anything that
 * looks like the original drawing.
 */

/** ACI 0..255 as 0xRRGGBB. Index 0 is BYBLOCK and 256 is BYLAYER (neither is a real colour). */
export const ACI_PALETTE: number[] = [
  ...
  0x333333, 0x505050, 0x696969, 0x828282, 0xbebebe, 0xffffff,
]

/** Colour index meaning "inherit from the block reference". */
export const COLOR_BYBLOCK = 0
/** Colour index meaning "inherit from the layer". */
export const COLOR_BYLAYER = 256

export function aciToRgb(index: number): number {
  if (index < 0) index = -index
  if (index === COLOR_BYLAYER) return 0xffffff
  return ACI_PALETTE[index] ?? 0xffffff
}
...
export function rgbToRgba(rgb: number, alpha: number): string {
  const r = (rgb >> 16) & 0xff
  const g = (rgb >> 8) & 0xff
  const b = rgb & 0xff
...
/** Perceptual luminance in 0..1, used to keep strokes readable on any backdrop. */
...
/**
 * CAD convention: the "white" pen prints black on paper and draws white on a
 * dark screen. Any colour that would vanish into the background is flipped the
 * same way, so a drawing authored for a black viewport stays legible on white.
 */
export function contrastAgainst(rgb: number, backgroundLuminance: number): number {
  const lum = luminance(rgb)
  if (Math.abs(lum - backgroundLuminance) > 0.28) return rgb
  return backgroundLuminance > 0.5 ? 0x000000 : 0xffffff
}
